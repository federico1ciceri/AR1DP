...
set.seed(48)
#change adjcompjitt and the file name in the other file
file="pam_adj.xlsx"
df<- adjcompjitt[ order(row.names(adjcompjitt)), ]  #sort alphabetically
...
for (i in 1:n_decade){      #iterate in the decades
  ...
  #some results and indices for the best clustering per each decade
  dunn_indeces[i]=intCriteria(matrix(data_decade),as.vector(pam_res$cluster),
                              "Dunn")[[1]]   #dunn index per each decade
  avg_sil_indeces[i]=best_msil    #average silhouette coefficient per each decade
  numb_of_clusters[i]=best_k      #number of clusters per each decade
...
gg_plot_adj_1900 <- ggplot(data_plot, aes(x = labels, y = df[,1])) + 
  geom_boxplot(width=0.5, color="black", alpha=0.6) + 
  ggtitle("Gender bias adj 1900 (PAM)") +geom_jitter(color= "red",width=0.15)+geom_hline(yintercept=0, color= "grey")

data_plot<-as.data.frame(df[,6])
data_plot$labels<-as.factor(cluster_labels[,6])
gg_plot_adj_1950 <- ggplot(data_plot, aes(x = labels, y = df[,6])) + 
  geom_boxplot(width=0.5, color="black", alpha=0.6) + 
  ggtitle("Gender bias adj 1950 (PAM)") +geom_jitter(color= "red",width=0.15)+geom_hline(yintercept=0, color= "grey")

data_plot<-as.data.frame(df[,11])
data_plot$labels<-as.factor(cluster_labels[,11])
gg_plot_adj_2000 <- ggplot(data_plot, aes(x = labels, y = df[,11])) + 
  geom_boxplot(width=0.5, color="black", alpha=0.6) + 
  ggtitle("Gender bias adj 2000 (PAM)") +geom_jitter(color= "red",width=0.15)+geom_hline(yintercept=0, color= "grey")
